...
/* Make SCHED_CBS a known sub-rq to the kernel */
...
#else
static inline bool cbs_policy(int policy)
{
	return false;
}
#endif
...
#ifdef CONFIG_MOKER_SCHED_CBS_POLICY
/* We add set, get, check param for setschedattr() */
extern int  sched_cbs_overflow(struct task_struct *p,
			       int policy,
			       const struct sched_attr *attr);
extern void __setparam_cbs(struct task_struct *p, const struct sched_attr *attr);
extern void __getparam_cbs(struct task_struct *p, struct sched_attr *attr);
extern bool __checkparam_cbs(const struct sched_attr *attr);
extern bool cbs_param_changed(struct task_struct *p, const struct sched_attr *attr);
#endif
...
#ifdef CONFIG_MOKER_SCHED_CBS_POLICY
	/*
	 * We add a new sub-rq that will hold RUNNABLE tasks of
	 * policy type SCHED_CBS
	 */
	struct cbs_rq		cbs;
#endif
