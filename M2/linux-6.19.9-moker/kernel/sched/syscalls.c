...
recheck:
...
#ifdef CONFIG_MOKER_SCHED_CBS_POLICY
	    (cbs_policy(policy) && !__checkparam_cbs(attr)) ||
#endif
	...
	if (unlikely(policy == p->policy)) {
		...
		if (dl_policy(policy) && dl_param_changed(p, attr))
			goto change;
#ifdef CONFIG_MOKER_SCHED_CBS_POLICY
		if (cbs_policy(policy) && cbs_param_changed(p, attr))
			goto change;
#endif
		if (attr->sched_flags & SCHED_FLAG_UTIL_CLAMP)
			goto change;
...
#ifdef CONFIG_MOKER_SCHED_CBS_POLICY
	if ((cbs_policy(policy) || task_has_cbs_policy(p)) &&
	    sched_cbs_overflow(p, policy, attr)) {
		retval = -EBUSY;
		goto unlock;
	}
#endif
...
#ifdef CONFIG_MOKER_SCHED_CBS_POLICY
	} else if (task_has_cbs_policy(p)) {
		__getparam_cbs(p, attr);
#endif
